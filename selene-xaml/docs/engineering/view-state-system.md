...
View runtime state 由 delivery boundary 私有持有。游戏只使用 Entity-based generated API
...
## Component Guest record

每个 Component instance 固定绑定一个 Selene World。Guest 在 linear memory 中以 Entity UInt 为
key 保存 Model、ViewModel 和 reconciliation state。不同 World 的相同 UInt 位于不同 instance，
不会发生 key 冲突。

Component Host 保存真实 Entity、instance/store 和 Selene UI Host record。Host 使用
`Entity::is_alive()` 做 GC：dead Entity 先触发 Guest `unmount(entity-id)`，再清理 Host tree；每帧
最后调用 Guest `update()`。

## Identity and reconciliation

两个 runtime 都遵守 ViewIR 确定的 identity policy：

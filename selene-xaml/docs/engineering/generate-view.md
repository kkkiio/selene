...
generator 原子生成一个 typed Embedded View package，不依赖 dependency package pre-build
执行任意命令。`.mbti` 是业务 contract 输入，每次 invocation 拥有一个 output directory 和
ownership lock。

## 分发

CLI 是 native executable，与 `infoset`、`parser`、`viewir` 和 `embedded_emitter` library
packages 保持在同一个 module 中原子发布；
...
Generated package 使用 `KKKIIO/selene/ui_view` 中的 mutation values 和进程内 `ViewHost`。
应用只依赖 Selene module，不需要把 compiler module 及其 `Milky2018/xml@0.4.0` parser
依赖带入应用 workspace。
...
`--mbti` 和 `--out-dir` 都是必需参数。CLI 只生成源代码，不运行 Moon build。

`--dry-run` 执行读取、解析、ViewIR lowering、emission 和格式化，向 stdout 输出将生成的
完整文件集合，不创建或修改 `--out-dir`。它用于验证完整 generation request；实际提交仍由不带
`--dry-run` 的同一命令执行。

存在多个 XAML document 时，由调用方分别执行命令并选择独立 output package。CLI 不接受内嵌
MoonBit expression、JSON expression map 或 generator manifest。

stdout 只输出请求结果，diagnostics 写 stderr，失败返回非零退出码。

## Input 与 lock 边界

XAML、MoonBit source 和派生 `.mbti` 是 source input。用户不维护 generator manifest；
state/patch/action mapping、package alias 和 output naming 由 source 推导。
...
| `.generated.mbt` | 是 | Embedded View source，供 review 与下游构建 |
...
自动发现。Selene 仓库 tracking `examples/inventory/src/view` 与

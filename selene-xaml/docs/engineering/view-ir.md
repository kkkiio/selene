...
typed semantic representation，由 Embedded emitter 读取。
...
每次 generator invocation 从 XAML 与 `.mbti` contract 构造一个 sealed `ViewIR`，随后生成一个
Embedded View package。
...
`.mbti` 解析成 compiler 内部的 resolved business contract。`ViewIR` 只保存当前 View 实际使用的
semantic type 和 symbol reference：
...
等语义 shape。MoonBit package alias 和 generated import name 属于 emitter 的 contract
projection，不参与 binding 或 reconciliation 语义判断。

Embedded generation 从 `.mbti` 构造 resolved contract，并保留 ViewModel field、Patch operation
和 Action variant 的 nominal MoonBit type。
...
UI store address 或 MoonBit source spelling。
...
Embedded emitter 将 route 生成成 Selene raw UI event 到 typed action EventBus 的映射，不重新解释
...
Embedded emitter 检查所需能力都有 concrete Selene API，并把 capability set 纳入 ABI fingerprint。
缺失能力是 generation error，不能静默跳过 property 或 event。
...
- Selene Entity object 的具体调用；
- EventBus 投影；
- source formatting 和 target-specific optimization。

Emitter 必须直接采用 ViewIR 已确定的 node identity、typed binding、structural scope、Action route
...
`ViewIR` 不保存 committed Model、candidate ViewModel、active branch、item order 或 Host Entity。
Generated source 使用 ViewIR 固化的控制流，在每次更新时创建和验证 `MutationPlan`。
...
emitter 之前完成。Embedded emitter 对 ViewIR behavior fixture 运行 semantic snapshot/compile
test。
...
保持 pure data，不实现 emitter method。

...
generated package 静态 import 它。XAML binding 只读取 ViewModel 字段；reconcile 比较前后两个
ViewModel，不分析用户函数体。
...
`ViewHost` 拒绝 mutation batch 时，committed ViewModel 和 reconciliation state 均保持不变。

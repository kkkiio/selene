...
| `ResourceDictionary` / `StaticResource` | resources / `{StaticResource}` | resources / `{StaticResource}` | resources / `{StaticResource}` | resources / `{StaticResource}` | design-token module / CSS custom property ~ |
...
XAML 保留 stable key 和声明关系，`selene/ui_view` 在提交完整 tree 后把方向目标解析为
Entity，并把 active scope 投影到 `selene/ui.focus_scopes()`。Generated View 不公开第二套
focus handle。View replacement 保留未替换节点的 Entity，scope 和导航连接随 mutation batch
重新投影。

Tab、Shift+Tab、方向键和 gamepad 输入最终调用设备无关的 `selene/ui.navigate()`。
...
| `Image` | 由 `Source` 加载图像，支持 `Fit`、`Tint`、`SourceRegion` 和 `atlas.json#region` | XAML `Image`；HTML `<img>` | 高 | `Tint` 与 `SourceRegion` 支持 literal、Binding 和 VisualState；`SourceRegion="x y width height"` 服务于外部 spritesheet metadata，空 Binding 清除裁剪。Selene `atlas` 可导入 canonical Selene、Aseprite 和 TexturePacker JSON。显式 `SourceRegion` 与 named atlas region 在同一 Image node 上互斥。 |
...
| `DataTemplate` | 保存一组可按 item data 实例化的 `Content`，当前由 `ItemsControl.ItemTemplate` 使用 | XAML `DataTemplate` | 高 | item 成为模板内容的 data scope；可在 `Resources` 中声明编译期资源，没有 `DataType`、template selector、hierarchical template 或任意框架对象的隐式模板查找。 |

## 组件化类型

| 类型 | Selene 定义 | 最接近参考物 | 一致程度 | 关键差异与取舍 |
| --- | --- | --- | --- | --- |
| `Component` | 可注册模板的根类型，声明 QName、parameters 和 content；实例拥有隔离的内部 namescope | XAML `UserControl`/custom control；Web Component | 中 | runtime 以精确 namespace URI + type name 注册，支持可选 spawn callback，并拒绝递归组件引用；可携带词法资源作用域，没有 CLR 类继承、code-behind、style selector 或 control-template 查找。 |
...
| `Setter` | 将 `Value` 写到 owner 或 `TargetName` 指向的节点属性 | WinUI visual-state `Setter`；XAML style setter | 高（WinUI）/中（WPF、Noesis） | 目标必须位于正确 namescope，属性必须属于 runtime 支持的 ECS 投影；`Value` 可使用编译期 `StaticResource`，没有样式 selector 和完整 dependency property precedence。 |
...
| `StaticResource` | `{StaticResource Key}` 从当前词法资源作用域读取标量设计 token | XAML `{StaticResource}` | 中 | 在 ViewIR lowering 前替换成带原使用位置 span 的字面量；没有 `DynamicResource` 或运行时 ambient lookup。 |

绑定扩展接受位置参数或 `Path=...`。`.` 和空路径表示当前 data 值；路径可
穿过 object 字段和 array 索引。

`ResourceDictionary` 支持 `Source`、`MergedDictionaries` 和带 `x:Key` 的
`Color`、`x:String`、`x:Double`、`x:Int32` 条目。外部字典在编译期读取并纳入
生成 ownership lock；运行时不保留资源查找表。
...
- **ECS-native runtime**：generated View 通过 `selene/ui_view` 创建 Entity 并写入 Selene UI stores，公开 View 身份始终是根 Entity。
...
- **轻量组件与状态系统**：typed parameters、slots、namescope、setter overlays 和有限 transition 覆盖声明式游戏 UI 的核心需求，避免引入 CLR reflection、完整 dependency property、运行时 style 和 Storyboard 子系统。
...
| `Text` | Accepted | `CG-VIEW-01`, `INVENTORY-01`, `VH-INCREMENTAL-01` |
| `Span` | Accepted | — |
| `Image` | Accepted | `CG-VIEW-01`, `INVENTORY-01`, `VH-VISUAL-STATE-01` |
| `Button` | Accepted | `CG-VIEW-01`, `INVENTORY-01`, `VH-EVENT-01`, `VH-VISUAL-STATE-01` |
| `Grid` | `PC-INFOSET-01` | `CG-GRID-01`, `INVENTORY-LAYOUT-01` |
...
| `ResourceDictionary` / `StaticResource` | `PC-RESOURCE-01` | `VI-RESOURCE-01`, `CG-RESOURCE-01` |
| `Component` / `Parameter` / `Slot` | Accepted | — |
| visual states 与 transitions | Accepted | `VI-IR-04`, `VH-VISUAL-STATE-01`, `INVENTORY-01` |
| focus scope、Tab 与方向导航 | Accepted | `EE-01`, `CG-FORMAT-01`, `VH-FOCUS-01`, `INVENTORY-LAYOUT-01` |
| `x:Reference` | Accepted | — |
| 热重载 | N/A | `VH-RELOAD-01` |
| UI mutation 增量提交 | N/A | `VH-INCREMENTAL-01` |
